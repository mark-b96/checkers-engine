...
class AI(object):
        ...
        if moves is not 0:  # Confirm that the piece can move
            for move in moves:
                ...
                if board_copy.legal_move(not max_player):
                    ...
                    board_copy = copy.deepcopy(board)  # Restore current board position
    ...
    @staticmethod
    def move_ordering(root, max_player):
        board = root.name
        all_possible_moves = []
    ...
    def heuristic_function(node):
        black_piece_count, white_piece_count, position_value = 0, 0, 0
        board = node.name
        for row in range(board.row_count):
            for column in range(board.column_count):
